...
import { UmbralAlerta } from '@/actions/alerta/getAlertaParametros';
...
export default function VisorDispositivo() {
  ...
  const [umbrales, setUmbrales] = useState<UmbralAlerta[]>([]);
  ...
  useEffect(() => {
    const loadUmbrales = async () => {
      try {
        const response = await fetch(`/api/umbrales?id_dispositivo=${dispositivo}`);
        const data = await parseJsonResponse<UmbralAlerta[]>(response, { fallback: [] });
        setUmbrales(data);
      } catch (error) {
      ...
      <div className="relative w-screen h-screen bg-white text-black text-center p-6">
        ...
        <div className="grid grid-cols-4 grid-rows-4 w-screen h-screen bg-white text-black text-center gap-6">
          {datosMostrar.map((dato, index) => {
            const thresholds = umbrales.find(u => u.parametro === dato.key);
